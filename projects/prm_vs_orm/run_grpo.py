...
def build_grpo_cmd(
    *,
    ...
    cmd: list[str] = [
        ...
        "--deepspeed_stage", "2",
        # NOTE: --gradient_checkpointing deliberately omitted. A 370M policy needs no activation
        # recompute to fit, and grad-ckpt's recomputed forward is one more place the DeepSpeed-bf16
        # stack can hand a Linear an fp32 weight against a bf16 activation. The universal F.linear
        # dtype-align guard (usercustomize written in main()) is the real fix; dropping this removes
        # a redundant risk surface. If memory ever forces it back, keep the guard.
...
# --------------------------------------------------------------------------------------------
# Universal dtype-align guard (the fix for the bf16-activation x fp32-weight crash).
# --------------------------------------------------------------------------------------------
# Both arms died with ``RuntimeError: expected mat1 and mat2 to have the same dtype, but got:
# c10::BFloat16 != float`` inside an Olmo3 ``F.linear`` (ORM at the MLP down_proj, PRM at attention
# q_proj -- nondeterministic location). Every offline hypothesis was refuted in the exact image env
# (transformers 5.4.0): all three models load clean bf16; the fp32 weight only appears once the live
# DeepSpeed-bf16 + Ray + (formerly) grad-checkpointing stack is running, which cannot be reproduced
# on a CPU laptop and lives entirely in the uneditable ``open_instruct``/DeepSpeed core. So rather
# than chase the exact upcast, neutralise its effect universally: align every ``nn.Linear`` weight
# to its input's dtype at call time. bf16 activation x stray-fp32 weight -> weight cast to bf16 ->
# a clean bf16 matmul (the intended training precision); when weights are already bf16 the guard is
# a no-op branch. This is the same overlay-monkeypatch idiom grpo_fast itself uses (it patches
# TorchCheckpointEngine.load). It is delivered via a generated ``usercustomize`` on PYTHONPATH so it
# activates at interpreter startup in EVERY worker -- the grpo_fast driver AND the DeepSpeed learner
# Ray actors where the crash actually happens (they need not import our reward plugin) -- gated
# behind an env var so only GRPO workers pay for it.
_DTYPE_GUARD_ENV = "PRM_VS_ORM_LINEAR_DTYPE_GUARD"

_DTYPE_GUARD_SRC = '''\
# AUTO-GENERATED by projects/prm_vs_orm/run_grpo.py -- do not edit. Universal Linear dtype-align
# guard. Delivered as BOTH sitecustomize.py and usercustomize.py on PYTHONPATH so ``site`` imports
# it at interpreter startup in EVERY worker (grpo_fast driver + the DeepSpeed learner Ray actors
# where the crash actually fires). sitecustomize is the load-bearing one: ``site`` skips
# usercustomize whenever ENABLE_USER_SITE is false, which is exactly the case inside the image's
# uv/venv (sys.prefix != sys.base_prefix), so a usercustomize-only guard would be silently inert and
# the crash would recur. sitecustomize has no such gate (only -S / -I skip it). Gated behind an env
# var so only GRPO workers pay the cost. See run_grpo.py for why.
import os as _os

# If the real venv ships its own sitecustomize.py we shadow it by sitting first on PYTHONPATH; run
# it too (best-effort, never fatal) so we never silently disable the image's site setup. Only the
# sitecustomize copy chains -- the usercustomize copy must not re-run it a second time.
try:
    import os as _os2, site as _site
    if _os2.path.basename(__file__) == "sitecustomize.py":
        _self = _os2.path.realpath(__file__)
        _dirs = list(getattr(_site, "getsitepackages", lambda: [])())
        try:
            _dirs.append(_site.getusersitepackages())
        except Exception:
            pass
        for _d in _dirs:
            _cand = _os2.path.join(_d, "sitecustomize.py")
            if _os2.path.isfile(_cand) and _os2.path.realpath(_cand) != _self:
...
if _os.environ.get("PRM_VS_ORM_LINEAR_DTYPE_GUARD") == "1":
    try:
        import torch.nn.functional as _F

        # Idempotent: sitecustomize and usercustomize can both fire (in non-venv workers); the
        # sentinel stops the second import from double-wrapping the already-patched F.linear.
        if not getattr(_F.linear, "_prm_vs_orm_guard", False):
            _orig_linear = _F.linear

            def _aligned_linear(input, weight, bias=None):
                # Align the weight (and bias) dtype to the input's. nn.Linear.forward calls
                # ``F.linear`` against this module object, so patching the attribute covers it.
            ...
            _aligned_linear._prm_vs_orm_guard = True
            _F.linear = _aligned_linear
            print("PRM_VS_ORM dtype guard installed (F.linear weight->input align)", flush=True)
    except Exception as _e:  # never let the guard break interpreter startup
        print("PRM_VS_ORM dtype guard NOT installed:", repr(_e), flush=True)
'''


def write_dtype_guard(overlay_dir: str) -> str:
    """Write the dtype-align guard as BOTH ``sitecustomize.py`` and ``usercustomize.py`` into
    ``overlay_dir``; return that dir.

    ``sitecustomize`` is the one that actually fires inside the image's uv/venv (``site`` skips
    ``usercustomize`` there because ENABLE_USER_SITE is false); ``usercustomize`` is a harmless
    fallback for any non-venv worker. Caller prepends the returned dir to PYTHONPATH and sets
    ``_DTYPE_GUARD_ENV=1`` so the guard arms in every worker interpreter. Kept pure/offline so
    ``_selftest`` can verify the emitted source without a GPU.
    """
    d = Path(overlay_dir)
    d.mkdir(parents=True, exist_ok=True)
    (d / "sitecustomize.py").write_text(_DTYPE_GUARD_SRC)
    (d / "usercustomize.py").write_text(_DTYPE_GUARD_SRC)
    return str(d)
...
def _selftest() -> None:
    ...
    # the dtype-crash fix: grad-checkpointing must NOT be launched (removes a redundant fp32-upcast
    # surface; the F.linear guard is the real fix). A regression here reintroduces a crash risk.
    assert "--gradient_checkpointing" not in cmd, "grad-checkpointing must stay off for this policy"

    # the universal dtype guard: writer must emit BOTH sitecustomize.py and usercustomize.py that
    # (a) are gated behind the env var and (b) patch F.linear to align weight dtype to the input's.
    # sitecustomize is load-bearing: usercustomize is inert in the image's venv (ENABLE_USER_SITE
    # false), so a usercustomize-only guard would silently never arm. Verify offline.
    import tempfile as _tf  # noqa: PLC0415 -- test-only
    with _tf.TemporaryDirectory() as _gd:
        gp = write_dtype_guard(_gd)
        assert (Path(gp) / "sitecustomize.py").is_file(), "guard must ship sitecustomize.py (venv-safe)"
        assert (Path(gp) / "usercustomize.py").is_file(), "guard must ship usercustomize.py (fallback)"
        src = (Path(gp) / "sitecustomize.py").read_text()
        assert _DTYPE_GUARD_ENV in src and "_F.linear = _aligned_linear" in src
        assert "weight.to(input.dtype)" in src, "guard must cast weight to the input's dtype"
        assert "_prm_vs_orm_guard" in src, "guard must be idempotent across the two import paths"
...
def main() -> None:
    ...
    # Arm the universal Linear dtype-align guard (fix for the bf16-activation x fp32-weight crash
    # that killed both arms). Write sitecustomize.py (+ usercustomize.py fallback) into a fresh dir
    # and PREPEND it to PYTHONPATH so ``site`` imports it at startup in the driver AND every
    # DeepSpeed learner Ray actor (which inherit PYTHONPATH via grpo_fast's ray.init runtime_env).
    # sitecustomize -- not usercustomize -- is what fires inside the image's uv/venv, where
    # ENABLE_USER_SITE is false. A separate dir (not REPO_ROOT) keeps this off the committed tree
    # and avoids assuming /opt/open-instruct is writable at runtime.
    guard_dir = write_dtype_guard("/tmp/prm_vs_orm_overlay")
    os.environ["PYTHONPATH"] = guard_dir + os.pathsep + os.environ["PYTHONPATH"]
    os.environ[_DTYPE_GUARD_ENV] = "1"
    print(f"armed Linear dtype-align guard via {guard_dir}/sitecustomize.py", flush=True)
